        ...
        while (!"Season end".equals(input = scanner.nextLine())) {
            if (input.contains(" -> ")) {
                ...
                if (!playerOne.equals(playerTwo) && playerInfo.containsKey(playerOne) && playerInfo.containsKey(playerTwo)) {
                    int totalPointsPlayerOne = 0;     //and both players exist in the tier, they duel with the following rules
                    int totalPointsPlayerTwo = 0;
                    for (var playerOneEntry : playerInfo.get(playerOne).entrySet()) {
                        for (var playerTwoEntry : playerInfo.get(playerTwo).entrySet()) {
                            if (playerOneEntry.getKey().equals(playerTwoEntry.getKey())) {  //if they got at least one in common !!!
                                totalPointsPlayerOne += playerOneEntry.getValue();         //the player with better total skill points wins
                                totalPointsPlayerTwo += playerTwoEntry.getValue();
                            }
                        }
                    }
                    if (totalPointsPlayerOne > totalPointsPlayerTwo) {   //the other is demoted from the tier -> remove him
